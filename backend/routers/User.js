...
const {
  ...
  router.post("/login", async (req, res) => {
    try {
      ...
      res
        ...
        .json({ success: 1, message: "Logged In Successfully" , token });
    } catch (err) {
      console.error(err);
      res.status(500).send();
  ...
  router.post("/logout", (req, res) => {
    res
      .cookie("token", "", { //1. overwrite the existing token cookie by setting it to an empty string.
        httpOnly: true,
        expires: new Date(0), //2. set the cookie to expire in the past (Jan 1, 1970)
      })
      .json({
        success: 1,
        message: "Logged Out",
  ...
  router.get("/isLoggedIn", (req, res) => {
    try {
      const token = req.cookies.token; //1. get the token from req
      

      if (!token) return res.send(false); //2. if !token send false
      jwt.verify(token, process.env.JWT_SECRET); //3. verify the token with the secret

      res.send(true); //4. send true
    } catch (err) {
      res.send(false); //5. check error
    }
  }),
  ...
  router.put("/", async (req, res) => {
    try {
      ...
      } //This prevents unauthorized users from making account changes.
      ...
      if (password && passwordVerify) { //case only for new password
        // new password
        if (password.length < 6) {
          return res.status(400).json({
            success: 0,
